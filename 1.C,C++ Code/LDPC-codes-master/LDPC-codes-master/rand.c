...
static int rn[N_tables][Table_size];
static int initialized = 0;
static rand_state state0;
...
v = this_nrand48(state->state48);
for (j = 0; j < N_tables; j++) v ^= rn[j][state->ptr[j]];

for (j = 0; j < N_tables && state->ptr[j] == Table_size - 1; j++)
state->ptr[j] = 0;

if (j < N_tables) state->ptr[j] += 1;
return v & 0x7fffffff;
}
...
double t = 0, r;
int i;
for (i = 0; i < n; i++) { if (p[i] < 0) abort(); t += p[i]; }
if (t <= 0) abort();
r = t * rand_uniform();
for (i = 0; i < n; i++) { r -= p[i]; if (r < 0) return i; }
for (i = 0; i < n; i++) if (p[i] > 0) return i;
abort();
}
...
int v = 0;
if (lambda > 10000)
return (int)(lambda + rand_gaussian() * sqrt(lambda) + 0.5);
while (1) {
lambda -= rand_exp();
if (lambda <= 0) break;
v++;
...
double rand_gaussian(void) {
double a = rand_uniform(), b = rand_uniopen();
return cos(2.0 * M_PI * a) * sqrt(-2.0 * log(b));
}
...
double b, c, X, Y, Z, U, V, W;
if (a < 0.00001) return a;
else if (a <= 1) {
U = rand_uniopen();
return rand_gamma(1 + a) * pow(U, 1 / a);
} else if (a < 1.00001) return rand_exp();
...
static int this_nrand48_r(unsigned short int xsubi[3], struct this_drand48_data* buffer, long int* result) {
if (this_drand48_iterate(xsubi, buffer) < 0) return -1;
if (sizeof(unsigned short int) == 2)
*result = xsubi[2] << 15 | xsubi[1] >> 1;
else
*result = xsubi[2] >> 1;
