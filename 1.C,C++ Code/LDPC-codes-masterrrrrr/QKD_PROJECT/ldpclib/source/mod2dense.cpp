/* MOD2DENSE.CPP - Procedures for handling dense mod2 matrices. */
...
)
{
  ...
  { printf("mod2dense_allocate: Invalid number of rows or columns\n");
    return NULL;
  }
...
{ 
  ...
  { printf("mod2dense_copy: Destination matrix is too small\n");
    return;
  }
...
{ 
  ...
  { printf("mod2dense_copyrows: Destination matrix has fewer columns than source\n");
    return;
  }
  ...
  { if (rows[i]<0 || rows[i]>=mod2dense_rows(m))
    { printf("mod2dense_copyrows: Row index out of range\n");
      return;
    }
    for (j = 0; j<mod2dense_cols(m); j++)
...
{ 
  ...
  if (mod2dense_rows(m)>mod2dense_rows(r))
  { printf("mod2dense_copycols: Destination matrix has fewer rows than source\n");
    return;
  }

  for (j = 0; j<mod2dense_cols(r); j++)
  { if (cols[j]<0 || cols[j]>=mod2dense_cols(m))
    { printf("mod2dense_copycols: Column index out of range\n");
      return;
    }
    for (k = 0; k<m->n_words; k++)
...
)
{
  ...
  { printf("mod2dense_get: row or column index out of bounds\n");
    return 0;
  }
...
{ 
  ...
  if (row<0 || row>=mod2dense_rows(m) || col<0 || col>=mod2dense_cols(m))
  { printf("mod2dense_set: row or column index out of bounds\n");
    return;
  }
...
)
{
  ...
  if (row<0 || row>=mod2dense_rows(m) || col<0 || col>=mod2dense_cols(m))
  { printf("mod2dense_flip: row or column index out of bounds\n");
    return 0;
  }
...
)
{
  ...
  { printf("mod2dense_transpose: Matrices have incompatible dimensions\n");
    return;
  }

  if (r==m)
  { printf("mod2dense_transpose: Result matrix is the same as the operand\n");
    return;
  }
...
)
{
  ...
  { printf("mod2dense_add: Matrices have different dimensions\n");
    return;
  }
...
)
{
  ...
  if (mod2dense_cols(m1)!=mod2dense_rows(m2) 
   ...
   || mod2dense_cols(m2)!=mod2dense_cols(r))
  { printf("mod2dense_multiply: Matrices have incompatible dimensions\n");
    return;
  }

  if (r==m1 || r==m2)
  { printf("mod2dense_multiply: Result matrix is the same as one of the operands\n");
    return;
  }
...
)
{
  ...
  { printf("mod2dense_equal: Matrices have different dimensions\n");
    return 0;
  }
...
)
{
  ...
  { printf("mod2dense_invert: Matrix to invert is not square\n");
    return 0;
  }

  if (r==m)
  { printf("mod2dense_invert: Result matrix is the same as the operand\n");
    return 0;
  }
  ...
  { printf("mod2dense_invert: Matrix to receive inverse has wrong dimensions\n");
    return 0;
  }
...
)
{
  ...
  if (r==m)
  { printf("mod2dense_invert_selected2: Result matrix is the same as the operand\n");
    return 0;
  }
  ...
  { printf("mod2dense_invert_selected2: Matrix to receive inverse has wrong dimensions\n");
    return 0;
  }
...
)
{
  ...
  if (mod2dense_rows(m)!=mod2dense_cols(m))
  { printf("mod2dense_forcibly_invert: Matrix to invert is not square\n");
    return 0;
  }

  if (r==m)
  { printf("mod2dense_forcibly_invert: Result matrix is the same as the operand\n");
    return 0;
  }
  ...
  if (mod2dense_rows(r)!=n || mod2dense_cols(r)!=n)
  { printf("mod2dense_forcibly_invert: Matrix to receive inverse has wrong dimensions\n");
    return 0;
  }
