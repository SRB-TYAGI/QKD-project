...
#include "../include/rcode.h"
#include "../include/fileio.h" // Include the correct header for open_file_std

void printoutError_read_pchk(const char* gen_file)
{
fprintf(stderr, "Error reading generator matrix from file %s\n", gen_file);
}

void printoutGarbled_read_pchk(const char* gen_file)
{
fprintf(stderr, "Garbled generator matrix in file %s\n", gen_file);
}
...
f = open_file_std(static_cast<const char*>(pchk_file), "rb");
if (f == NULL)
{
    printf("Can't open parity check file: %s\n", pchk_file);
    return;
...
void rCode::read_gen(char* gen_file, int cols_only, int no_pchk_file)
{
int M2, N2;
FILE* f;
int i;
...
f = open_file_std(static_cast<const char*>(gen_file), "rb");
if (f == NULL)
{
    printf("Can't open generator matrix file: %s\n", gen_file);
    return;
