/* MOD2SPARSE.CPP - Procedures for handling sparse mod2 matrices. */
...
{ 
  ...
  { printf("mod2sparse_copyrows: Destination matrix has fewer columns than source\n");
    return;
  }
...
{ 
  ...
  if (mod2sparse_rows(m)>mod2sparse_rows(r))
  { printf("mod2sparse_copycols: Destination matrix has fewer rows than source\n");
    return;
  }
...
)
{
  ...
  { printf("mod2sparse_transpose: Matrices have incompatible dimensions\n");
    return;
  }

  if (r==m)
  { printf("mod2sparse_transpose: Result matrix is the same as the operand\n");
    return;
  }
...
)
{
  ...
  if (r==m1 || r==m2)
  { printf("mod2sparse_add: Result matrix is the same as one of the operands\n");
    return;
  }
...
)
{
  ...
  if (mod2sparse_cols(m1)!=mod2sparse_rows(m2) 
   ...
   || mod2sparse_cols(m2)!=mod2sparse_cols(r))
  { printf("mod2sparse_multiply: Matrices have incompatible dimensions\n");
    return;
  }

  if (r==m1 || r==m2)
  { printf("mod2sparse_multiply: Result matrix is the same as one of the operands\n");
    return;
  }
...
)
{
  ...
  { printf ("mod2sparse_add_row: row added to is shorter than row added from\n");
    return;
  }
...
)
{
  ...
  if (mod2sparse_rows(m1)<mod2sparse_rows(m2))
  { printf("mod2sparse_add_col: Column added to is shorter than column added from\n");
    return;
  }
...
{  
  ...
  { printf("mod2sparse_decomp: Matrices have incompatible dimensions\n");
    return 0;
  }

  if (abandon_number>N-K)
  { printf("Trying to abandon more columns than allowed\n");
    return 0;
  }
  ...
  { 
    ...
    if (found)
    { 
      if (cinv[mod2sparse_col(e)]!=k) return 0;
    ...
    { 
      ...
      if (cc2!=abandon_number) return 0;
...
)
{
  ...
  { ii = rows ? rows[i] : i;
    ...
    { printf("mod2sparse_forward_sub: Matrix is not lower-triangular\n");
      return 0;
    }
  }
...
)
{
  ...
  { ii = cols ? cols[i] : i;
    ...
    { printf("mod2sparse_backward_sub: Matrix is not upper-triangular\n");
      return 0;
    }
  }
